...
function renderBioButton(m) {
  ...
  const paragraphs = m.bio
    ...
    .join('');
  return `
    <button type="button" class="pt-team-more-btn" data-bio-open="${bioId}" data-bio-name="${m.name}" data-bio-role="${m.role}" data-bio-photo="${m.photo ? urlFor(m.photo).url() : ''}">
        Know More <i class="fas fa-arrow-right" aria-hidden="true"></i>
    </button>
    <template id="${bioId}">${paragraphs}</template>
  `;
}
